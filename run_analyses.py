...
if test_path_convergence:
    ...
    for r in range(replicates):
        ...
        for i,N in tqdm(enumerate(n_paths)):
        ...
        ax1.plot(n_paths, effective_barriers + R*T*np.log(n_paths))
        ax2.plot(n_paths, permeabilities / n_paths)

  
    ax1.xlabel('number of paths')
    ax1.ylabel('effective barrier')

    ax2.xlabel('number of paths')
    ax2.ylabel('permeability per path')
    plt.show()